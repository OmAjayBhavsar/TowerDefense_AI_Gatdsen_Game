package com.gatdsen.ui.menu;

import com.gatdsen.ui.GADS;
import com.gatdsen.ui.menu.attributes.Attribute;
import com.gatdsen.ui.menu.attributes.IpAdressReadAttribute;
import com.gatdsen.ui.menu.attributes.PlayerAttribute;
import com.gatdsen.ui.menu.attributes.PortReadAttribute;

public class MultiplayerHostScreen extends AttributeScreen{
    /**
     * Konstruktor für die Klasse AttributeScreen
     *
     * @param gameInstance Eine Instanz des GADS-Spiels
     */
    public MultiplayerHostScreen(GADS gameInstance) {
        super(gameInstance);
    }

    @Override
    protected Attribute[] getAttributes() {
        return new Attribute[]{new PlayerAttribute(0), new IpAdressReadAttribute(), new PortReadAttribute()};
    }

    @Override
    String getTitelString() {
        return "Mehrspieler Spiel erstellen";
    }

    @Override
    GADS.ScreenState getNext() {
        return GADS.ScreenState.INGAMESCREEN;
    }

    @Override
    GADS.ScreenState getPrev() {
        return GADS.ScreenState.MAINSCREEN;
    }
}