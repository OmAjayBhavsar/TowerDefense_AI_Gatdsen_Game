...
public final class state {
    ...
    state(GameState state, int playerIndex, Timer timer) {
        this.state = state;
    ...
    /**
     * Gibt den Turm an der angegebenen Position zurück, oder null, falls kein Turm an dieser Position steht.
     * @param x x-Koordinate, an der sich der Turm befindet
     * @param y y-Koordinate, an der sich der Turm befindet
     * @return Der Turm an der angegebenen Position, oder null, falls kein Turm vorhanden ist
     */
    public Tower getTower(int x, int y) {
        if (x >= 0 && x < getBoardSizeX() && y >= 0 && y < getBoardSizeY()) {
            return state.getTower(x, y);
        }
        return null;
    }
    ...
    public enum OpponentType {EMP_ENEMY, ARMOR_ENEMY, SHIELD_MOUSE}
}
