 ...
 */
public final class Campaign2_2Bot extends Bot {
    ...
    public void executeTurn(state state, Controller controller) {
        switch (counter) {
            case 0:
                if (state.getMyPlayerState().getMoney() > Tower.getTowerPrice(TowerType.MINIGUN_CAT)) {
                    controller.placeTower(6, 9, TowerType.MINIGUN_CAT);
                    counter++;
                }
                break;
            case 1:
                if (state.getMyPlayerState().getMoney() > Tower.getTowerPrice(TowerType.CATANA_CAT)) {
                    controller.placeTower(6, 7, TowerType.CATANA_CAT);
                    counter++;
                }
                break;
            case 2:
                if (state.getMyPlayerState().getMoney() > Tower.getTowerPrice(TowerType.MAGE_CAT)) {
                    controller.placeTower(5, 4, TowerType.MAGE_CAT);
                    counter++;
                }
                break;
        }
